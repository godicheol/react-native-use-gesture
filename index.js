...
function useGesture() {
    ...
    function returnSwipe() {
        ...
        velocityX = Math.abs(moveX)/(currentTime-initialTime);
        velocityY = Math.abs(moveY)/(currentTime-initialTime);
        if (callbacks.swipe) {
            callbacks.swipe({
                initialX: initialTouches[0].pageX,
                initialY: initialTouches[0].pageY,
    ...
    function returnRotate() {
        ...
        if (callbacks.rotate) {
            callbacks.rotate({
                degree: ((Math.max(d1, d2) % 360) + 360) % 360,
                degree1: ((d1 % 360) + 360) % 360,
                degree2: ((d2 % 360) + 360) % 360,
                initialTime: initialTime,
                currentTime: currentTime,
            });
        }
